...
contract GarbageSaleTestSuit is TestHelper {
    ...
    function test_SetUpState(address _user) public {
        assertEq(address(saleContractV1.priceFeed()), address(priceFeed));
        assertEq(saleContractV1.tokenPrice(), tokenPrice);
        assertEq(saleContractV1.saleLimit(), saleLimit * 1e18);
        (uint256 ethSpent, uint256 tokensPurchased) = saleContractV1.users(_user);
        assertEq(ethSpent, 0);
    ...
    function test_Purchase_OK_SinglePurchase(address _user, uint256 _randomUint) public {
        ...
        (, uint256 tokensAmount) = saleContractV1.convertETHToTokensAmount(ethAmount);
    ...
    function test_Purchase_OK_SeveralPurchases(address _user, uint256 _randomUint) public {
        ...
        (ethSpent, tokensPurchased) = saleContractV1.users(_user);
    ...
    function test_Purchase_Revert_PerWalletLimitExceeded_SeveralPurchases(address _user, uint256 _randomUint) public {
        ...
        vm.prank(_user);
        payable(address(saleContractV1)).call{ value: ethAmount / 2 }("");
    ...
    function test_Purchase_Revert_SaleLimitExceeded(address _user, uint256 _randomUint) public {
        ...
        saleContractV1.setSaleLimitHarness(tokensAmount - 1);
    ...
    function test_Purchase_convertETHToTokensAmount_OK(uint256 _ethAmount, uint256 _ethPrice) public {
        ...
        (uint256 ethPrice, uint256 tokensAmount) = saleContractV1.convertETHToTokensAmount(_ethAmount);

        assertEq(ethPrice, _ethPrice * 1e6);
        assertEq(tokensAmount, _ethAmount * _ethPrice * 1e6 / saleContractV1.tokenPrice());
    }
}
